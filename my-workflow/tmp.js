...
var safeJsonStringify = (obj) => JSON.stringify(obj, (_, value) => typeof value === "bigint" ? value.toString() : value, 2);
...
var asAddress = (value2) => {
  if (!value2 || !/^0x[a-fA-F0-9]{40}$/.test(value2)) {
    return ZERO_ADDRESS;
  }
  return value2;
};
var asBigIntWithDefault = (value2, fallback) => {
  if (!value2) {
    return fallback;
  }
  try {
...
var parseSlippageBps = (value2) => {
  const bps = typeof value2 === "number" && Number.isFinite(value2) ? Math.max(1, Math.floor(value2)) : 100;
  return BigInt(Math.min(500, bps));
};
var parseMinConfidence = (value2, fallback) => {
  if (value2 === undefined) {
    return fallback;
  }
...
var parseGmxActionFromModel = (value2) => {
  if (typeof value2 !== "string") {
    return;
  }
  return normalizeTextAction(value2);
};
var resolveSwitcherAction = (forcedTradingAction, modelAction, strategy, rationale) => {
...
var mergeIntentConfig = (globalIntent, chainIntent) => ({
...
var withWethUsdcDefaults = (intent) => ({
...
var buildGmxIntentPayload = (action, summary, confidence, globalIntent, chainIntent) => {
  ...
  const acceptablePrice = action === "long" ? triggerPrice * (10000n + slippageBps) / 10000n : triggerPrice * (10000n - slippageBps) / 10000n;
  return {
...
var tryExecuteGmxIntent = (runtime2, receiver, action, intent) => {
  ...
  try {
    runtime2.log(`[GMX payload] action=${action} sizeUsd=${intent.sizeUsd.toString()} triggerPrice=${intent.triggerPrice.toString()} acceptablePrice=${intent.acceptablePrice.toString()}`);
...
var recommendForChain = (runtime2, httpClient, chain, coingeckoApiKey, geminiApiKey, options) => {
  ...
  const ohlc = JSON.parse(serializedOhlc);
  ...
  const prompt = buildPrompt(chain, summary, ohlc, options.forcedTradingAction, runtime2.now().toISOString());
  ...
  runtime2.log(`[OPERATION_LOG] ${JSON.stringify(operationLog)}`);
  return {
    chain: chain.chain,
    ...
    riskControls: Array.isArray(parsedGemini.riskControls) ? parsedGemini.riskControls.slice(0, 3) : ["Use stop-loss", "Limit position size", "Review volatility"],
    route: {
...
var runStrategyFlow = (runtime2, reason, mode, options) => {
  runtime2.log("━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━");
  runtime2.log("CRE Workflow: CoinGecko OHLC + Gemini Strategy Recommender");
  runtime2.log(`Reason: ${reason}`);
  runtime2.log(`Mode: ${mode}`);
  const coingeckoApiKey = runtime2.getSecret({ id: runtime2.config.coingeckoApiKeySecretId }).result().value;
  const geminiApiKey = runtime2.getSecret({ id: runtime2.config.geminiApiKeySecretId }).result().value;
  const httpClient = new ClientCapability3;
  const chainsToRun = runtime2.config.chains.filter((chain) => {
    if (mode === "position") {
  ...
  const recommendations = chainsToRun.map((chain) => {
    runtime2.log(`Processing ${chain.chain} (${chain.coingeckoCoinId})`);
    const recommendation = recommendForChain(runtime2, httpClient, chain, coingeckoApiKey, geminiApiKey, options);
    if (recommendation.target === "gmx") {
      runtime2.log(`[GMX] action=${recommendation.gmx.action} shouldExecute=${String(recommendation.gmx.shouldExecute)} status=${recommendation.gmx.status} detail=${recommendation.gmx.detail}`);
    }
  ...
  runtime2.log(`Generated ${recommendations.length} recommendations`);
  runtime2.log("━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━");
  return result;
};
var parseTradingAction = (value2) => {
  if (value2 === undefined) {
    return "auto";
  }
...
var onHttpTradingTrigger = (runtime2, payload) => {
  ...
  const reason = typeof parsed.reason === "string" && parsed.reason.trim().length > 0 ? parsed.reason.trim() : "http-trading";
  ...
  runtime2.log(`[HTTP trading] action=${forcedTradingAction} minConfidence=${minConfidence}`);
  return runStrategyFlow(runtime2, reason, "trading", {
    forcedTradingAction,
    minConfidence
  });
};
